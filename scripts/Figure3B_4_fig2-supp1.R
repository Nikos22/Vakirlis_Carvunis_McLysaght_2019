...
df <- read.csv("Figure3-supplement_1.csv",
                row.names=NULL,
...
df[df$tag=="fruitfly", "tag"] <- "fly"
df$tag <- factor(df$tag, levels=c("yeast", "fly", "human"), ordered = TRUE)
...
ggsave("figures/Figure2-supplement_1.pdf", height=10, width=5)
...
ggsave("figures/Figure3B.png", width=6, height=4.5)
...
drosCor <- cor.test(with(filter(df, tag=="fly"), div), with(filter(df, tag=="fly"), no_match/total), method="pearson")
...
ggsave(plot = div_fn, "figures/Figure4.pdf", width=8, height=6)